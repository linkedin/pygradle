...
def escape_description(desc):
  ...
  # Descriptions are waaay too long.  Kill everything after the first sentence.
  desc = re.sub('\.\s.*$', '', desc)
  return desc
  ...
  for param in click_command.params:
    if isinstance(param, click.Option):
      for opt in param.opts:
        ...
        results.append('"{multiple}{opt}[{desc}]{spec}"'.format(opt=opt,
                                                                desc=desc,
                                                                spec=spec,
  ...
  for param in click_command.params:
    if isinstance(param, click.Argument):
      for opt in param.opts:
        desc = param.make_metavar()
        # todo nargs
        results.append('": : _message \'{opt} = {desc}\'"'.format(opt=opt, desc=desc))
  return ' '.join(results)
...
def zsh_generate_group_completions(click_group):
  ...
  result.append('''_arguments -S -C "*:: :->{name}_subcommand" {subcommands} {optionspecs}
  if [[ $state == {name}_subcommand ]]
  then
  '''.format(name=click_group.name,
...
def zsh_generate_command_completions(click_command):
  ...
  """
  return '_arguments -S {optionspecs}'.format(optionspecs=zsh_generate_optionspecs(click_command))
...
def bash_generate_completions(click_obj):
  ...
  if is_group(click_obj):
    ...
    result.append('COMPREPLY=($(compgen -W "{words}" -- ${{cur}} ))'.format(words=' '.join(words)))
    result.append(';;')
    result.append('esac')
  elif is_command(click_obj):
    words = []
...
def fish_generate_completions(click_obj, cmdname, topname):
  ...
  prefix = '''complete -c {topname} -n '{condition}' '''.format(topname=topname, condition=condition)
  if is_group(click_obj):
    for param in click_obj.params:
      if hasattr(param, 'opts'):
...
def main():
  ran = False
  for name, entrypoint in list(pkg_resources.working_set.__iter__().next().get_entry_map(group='console_scripts').items()):
    try:
      ep = entrypoint.load()
      if is_command(ep):
        generate_completions(name, ep)
        print("Generating completions for %s..." % name)
        ran = True
      else:
        print("%s is not Click based, skipping." % name)
    except ImportError as e:
      print("ERROR: Cannot generate completions for %s; see stacktrace below." % name)
      print("#" * 80)
      logging.exception(e)
      print("#" * 80)
      continue
